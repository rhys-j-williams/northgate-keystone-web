...
# The @northgate scope in .npmrc points at the local Verdaccio for local builds; in CI the
# secret-mounted .npmrc overrides it with Artifactory and the token (GIS-2911, never an ARG).
RUN --mount=type=secret,id=npmrc,target=/root/.npmrc \
    if [ -n "${NPM_REGISTRY}" ]; then npm config set @northgate:registry "${NPM_REGISTRY}"; fi \
...
LABEL org.opencontainers.image.title="keystone-web" \
      ...
      bank.northgate.team="identity-platform" \
      bank.northgate.ticket-key="KEY"
